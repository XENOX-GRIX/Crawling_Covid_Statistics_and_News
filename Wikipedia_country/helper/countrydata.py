...
global_output_file = open("yoyo.txt", 'w', encoding="utf-8")
###DEFINING TOKENS###
tokens = ('FIRST',
          'SECOND',
          'CONTENT',
...
def p_content_items(p):
    '''content_items : handle_date content_item content_items
                     | content_item content_items
                     | '''

def p_handle_date(p):
    '''
    handle_date : FIRST CONTENT 
    '''
    global x 
    global s
    global date
...
def crawl_country():
    country_file = './Pages/extracted_country_data.txt'
    output_dir = os.path.join('Pages', 'country_data')
