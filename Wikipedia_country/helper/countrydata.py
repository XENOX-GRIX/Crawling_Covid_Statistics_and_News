...
def process_html_page(html_page_name, output_dir):
    ...
    # Define the output file path within the specified output directory
    output_file_path = os.path.join(output_dir, os.path.basename(html_page_name).replace('.html', '.txt'))
    
    # Open the output file for writing and assign it to the global variable
    global_output_file = open(output_file_path, 'w', encoding="utf-8")
    # Open the HTML file to read data
    ...
    with open(output_file_path, 'w+', encoding="utf-8") as output_file:
        ...
        parser = yacc.yacc()
        # Modify the parser to accept the output_file as an argument
        parser.parse(data)
        global_output_file.close() 

def crawl_country():
    country_file = 'extracted_country_data.txt'
    output_dir = os.path.join('helper', 'country_data')
    
    # Ensure the output directory exists
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    # Read the timeline file and process each HTML page
    with open(country_file, 'r', encoding="utf-8") as file:
        for line in file:
            html_page_name = line.strip()  # Remove any leading/trailing whitespace
